...
class XtdCommentForm(CommentForm):
    # Translators: Перевести как Notify me about follow-up comments
    followup = forms.BooleanField(required=False,
                                  label=_("Сообщите мне о последующих комментариях"))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['name'] = forms.CharField(
            widget=forms.TextInput(attrs={'placeholder': _('Имя'), 'class':'form-control'}))
        # Translators: Required for comment verification
        self.fields['email'] = forms.EmailField(
            label=_("Email"), help_text=_("Требуется для проверки комментариев"),
            widget=forms.TextInput(attrs={'placeholder': _('Email'), 'class':'form-control'})
            )
        self.fields['url'] = forms.URLField(
            required=False,
            widget=forms.TextInput(attrs={'placeholder': _('Вебсайт'), 'class':'form-control'}))
        self.fields['comment'] = forms.CharField(
            widget=forms.Textarea(attrs={'placeholder': _('Комментарий'), 'class':'form-control'}),
